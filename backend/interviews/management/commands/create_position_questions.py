...
Usage: python manage.py create_position_questions
"""
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            '--position',
            type=str,
            choices=['virtual_assistant', 'customer_service', 'it_support', 'sales_marketing', 'general', 'all'],
            default='all',
            help='Position type to create questions for'
        )
    ...
    def handle(self, *args, **options):
        ...
        positions_to_create = [position] if position != 'all' else questions_data.keys()
        
        created_count = 0
        skipped_count = 0
        
        for pos_type in positions_to_create:
            self.stdout.write(f'\nProcessing {pos_type.replace("_", " ").title()} questions...')
            
            for q_data in questions_data[pos_type]:
                # Check if question already exists
                if InterviewQuestion.objects.filter(
                    question_text=q_data['question_text'],
                    position_type=pos_type
                ).exists():
                    self.stdout.write(self.style.WARNING(f'  ⊘ Skipped (already exists): {q_data["question_text"][:60]}...'))
                    skipped_count += 1
                    continue
        ...
        self.stdout.write(f'  Total in database: {InterviewQuestion.objects.count()} questions')
        
        # Show breakdown by position
        self.stdout.write(f'\n  Breakdown by position:')
        for pos_choice in InterviewQuestion.POSITION_TYPE_CHOICES:
            pos_code = pos_choice[0]
            pos_name = pos_choice[1]
            count = InterviewQuestion.objects.filter(position_type=pos_code).count()
            self.stdout.write(f'    • {pos_name}: {count} questions')
